...
use App\HHH_Library\ThisApp\API\JustCall\Enums\ErrorEnum;
...
abstract class SuperApiRequest
{
    ...
    public function send(): self
    {
        ...
        $requestHandler = new ApiRequestHandler($requestUrl);
    ...
    public function getStatus(): ApiStatusEnum
    {
        if (JsonResponseHelper::isJsonResponseSuccess($this->getResponse())) {
            ...
            return ($this->getResponseCode() === "success") ? ApiStatusEnum::Success : ApiStatusEnum::FailedExternal;
        } else
    ...
    public function getErrorMessage(): ?string
    {
        ...
        if ($status === ApiStatusEnum::FailedInternal->name) {
            ...
            return ErrorEnum::findMessage($msg);
        } else
            return __('JustCallApi.UnknownError');
    }

    /*************************** request common getter functions  END ***************************/

    /**
     * Modify phone number format for JustCall
     *
     * @param  ?string $phoneNumber
     * @return string
     */
    protected function modifyPhoneNumberFormat(?string $phoneNumber): string
    {
