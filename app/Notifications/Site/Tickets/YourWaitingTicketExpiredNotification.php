...
class YourWaitingTicketExpiredNotification extends SuperDatabaseNotification
{
    use Queueable;

    private $ticketId;
    private $maxWaitingHours;
     ...
     * @return void
     */
    public function __construct(int $ticketId, int $maxWaitingHours)
    {
        $this->ticketId = $ticketId;
        $this->maxWaitingHours = $maxWaitingHours;
    }
    ...
    public function toDatabase(object $notifiable): array
    {
        return [
            ...
            'maxWaitingHours'   => $this->maxWaitingHours,
        ];
    }
    ...
    {
        return trans('notifications.YourWaitingTicketExpired.subject');
    }
        ...
        try {
            if ($notification = Notification::find($notificationId)) {
                ...
                if ($ticket = Ticket::find($ticketId)) {
                    ...
                    $message = trans(
                        'notifications.YourWaitingTicketExpired.message',
                        [
                            'ticketId'        => $ticketId,
                            'maxWaitingHours' => $notificationData['maxWaitingHours'],
                        ]
                    );
